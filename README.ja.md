...
Loomit は型紙ツールチェーンの3分の1で、それぞれ仕事は1つだけです。

| ツール                                                  | 仕事                                             |
| ------------------------------------------------------- | ------------------------------------------------ |
| **Loomit**（このリポジトリ）                            | 型紙の構造・組み立てグラフ・意味的 `diff`。      |
| **[Seamlint](https://github.com/kana001-bit/Seamlint)** | 幾何を測り、問題を報告する。                     |
| **[Truer](https://github.com/kana001-bit/Truer)**       | 直し方を提案し、人が受け入れたものだけを書く。   |

境界は内部実装より先に決めてあります。Loomit が**何と何が繋がるか**を、Seamlint が**何が間違っているか**を、Truer が**どう直しうるか、そしてそれを書くこと**を決め、**実際に何を変えるかは人が決めます**。Loomit は幾何を計算せず、型紙を書き換えもしません。測定は Seamlint へ、修正は Truer へ request として渡し、その答えを読み戻すだけです。受け渡し点は `loom slnt check` と `loom match` の2つ（[CLI リファレンス](docs/cli.md)）。
